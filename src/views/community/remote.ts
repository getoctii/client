...
export const getCommunity = async (communityID: string, token: string) =>
  (
...
export const getChannels = async (communityID: string, token: string) =>
  (
    await clientGateway.get<string[]>(`/communities/${communityID}/channels`, {
      headers: {
        Authorization: token
      }
    })
