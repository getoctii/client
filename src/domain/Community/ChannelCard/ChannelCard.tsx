...
const ChannelCardView: FC<{
  ...
  const { channelID } = matches.find((match) => !!match.params.channelID)
      ...
      items.push(
        {
          ...
          onClick: () =>
            navigate({
              to: `/app/communities/${community?.id}/channels/${id}/settings`
            })
        },
        {
          text: 'Delete Channel',
        ...
        <div
          ...
          onClick={() => {
            ...
            if (channel.type === ChannelTypes.TEXT)
              return navigate({
                to: `/app/communities/${community?.id}/channels/${channel.id}`
              })
